...
| `sudo sed -i '/ swap / s/^/#/' /etc/fstab` | Disable swap permanently | Prevent swap after reboot | `cat /etc/fstab` |
| `/etc/modules-load.d/containerd.conf` | Load `overlay` & `br_netfilter` | Needed for networking | `lsmod | grep br_netfilter` |
